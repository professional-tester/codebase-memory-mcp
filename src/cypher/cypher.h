...
typedef struct {
     ...
     * (upstream 755ffe57/#874). NULL func = plain variable/property LHS. */
    const char *func;
    cbm_func_arg_t *args;
    int arg_count;
} cbm_condition_t;
 ...
 * remainder (upstream 0a0ef516/#1334 — the unconsumed tail swallowed LIMIT). */
...
typedef struct {
     ...
     * length hop range was clamped to the engine ceiling (upstream
     * 73a3c347/#797) — without this, a clamped expansion is
     * indistinguishable from "no such path". */
    char *warning;
} cbm_cypher_result_t;
