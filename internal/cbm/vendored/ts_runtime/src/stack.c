...
// CBM patch (upstream da046da5, #9): cap the recursive ambiguity-merge in
// stack_node_add_link. On a deeply ambiguous parse (e.g. Perl's optional-paren
// function calls in a f(f(f(...))) chain ~30k deep), the "merge them
// recursively" path below recurses once per nesting level on the C stack,
// overflowing the small default thread stack on Windows (~1 MB) and even the
// 8 MB POSIX stack at extreme depth — a SIGSEGV during ts_parser_parse,
// before any language extractor runs. Past this cap we stop eagerly merging
// and leave the ambiguity on the GLR stack, exactly as the existing
// `self->link_count == MAX_LINK_COUNT` bail-out already does: the parse still
// produces a valid tree (graceful degradation), never a wrong one.
